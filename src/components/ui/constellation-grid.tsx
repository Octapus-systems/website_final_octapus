...
interface ConstellationGridProps {
    className?: string;
    /** Override accent color as "r, g, b" */
    accent?: string;
}

export default function ConstellationGrid({ className, accent }: ConstellationGridProps) {
    const canvasRef = useRef<HTMLCanvasElement | null>(null);
    const [isDarkMode, setIsDarkMode] = useState<boolean>(true);

    // Sync with the app's dark class on <html>
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            // Octapus-branded palette
            const bgColor = isDarkMode ? '#030407' : '#fafafa';
            const nodeColor = isDarkMode ? '255, 255, 255' : '15, 23, 42';
            // Default accent: Octapus purple (oklch(0.62 0.2 285) ≈ rgb(96, 28, 230))
            const accentColor = accent ?? (isDarkMode ? '139, 92, 246' : '96, 28, 230');
    ...
    return (
        <canvas
            ref={canvasRef}
            className={`absolute inset-0 block cursor-crosshair ${className ?? ''}`}
        />
    );
}
